...
func extractNodeIPFromFlag(generatedKubelet kubelet.Kubelet) (net.IP, error) {
	kubeletEnv := generatedKubelet.GetEnv(kubelet.KubeletArgsEnvironmentName)
...
func extractHostName(generatedKubelet kubelet.Kubelet) (string, error) {
	kubeletEnv := generatedKubelet.GetEnv(kubelet.KubeletArgsEnvironmentName)
...
func getNodeName(generatedKubelet kubelet.Kubelet) (string, error) {
	return extractHostName(generatedKubelet)
}
...
func getNodeIP(generatedKubelet kubelet.Kubelet) (net.IP, error) {
	...
	nodeIP, err := extractNodeIPFromFlag(generatedKubelet)
	if err != nil {
		return nil, err
	}
	hostname, err := extractHostName(generatedKubelet)
	if err != nil {
		return nil, err
	}
	...
	if nodeIPSpecified {
		...
		var addrs []net.IP
		nodeName, err := getNodeName(generatedKubelet)
		if err != nil {
			return nil, err
		}
...
func ValidateNodeIp(ctx context.Context, provider nodeprovider.NodeProvider, kubelet kubelet.Kubelet) error {
