...
do
    nodeadm install $VERSION  --credential-provider ssm --containerd-source docker

    assert::path-exists /usr/bin/containerd
    assert::containerd-installed-from-docker true
    ...
    assert::assert::containerd-installed-from-docker false
